...
class DbHelper (var mContext: Context) :
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        Log.d("abc", "onCreate")
        val createTable =
            "create table $TABLE_NAME ($COLUMN_ID integer, $COLUMN_TITLE char(50), $COLUMN_DESCRIPTION char(200))"
    ...
    fun addTask(addTask: ToDo) {
        var sqLiteDatabase = writableDatabase
        var contentValues = ContentValues()
        contentValues.put(COLUMN_ID, addTask.id)
        contentValues.put(COLUMN_TITLE, addTask.title)
        contentValues.put(COLUMN_DESCRIPTION, addTask.description)
